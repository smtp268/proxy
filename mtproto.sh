...
# bash <(curl -sL https://git.io/mtproto.sh)
...
installDocker() {
    ...
    DOCKER_CMD="$(command -v docker)"
    if [[ "$DOCKER_CMD" = "" ]]; then
        echo -e " ${RED}$OSNAME docker安装失败，请到 https://t.me/Tony_Chat_bot 反馈${PLAIN}"
        exit 1
    fi
    systemctl enable docker
    systemctl start docker
...
start() {
    ...
    if [[ "$res" = "" ]]; then
        ...
        echo -e " ${RED}$OSNAME 启动docker镜像失败，请到  https://t.me/Tony_Chat_bot 反馈${PLAIN}"
        exit 1
    else
...
menu() {
    clear
    echo "#############################################################"
    echo -e "# ${YELLOW}✅ MTProto一键安装脚本 😄${PLAIN}       #"
